...
//  TreeMap_AVL.h
//
//  ImplementaciÛn de diccionarios <clave, valor> como ·rboles de b˙squeda AVL
//
...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(clave_valor const& e, Link i = nullptr, Link d = nullptr,
                 int alt = 1, int tam = 1) : tam_i(tam), cv(e), iz(i), dr(d), altura(alt) {}
    };
    ...
    // objeto funciÛn que compara elementos
    Comparador menor;
...
public:
    ...
    //Método para buscar el k-esimo elemento, con coste log(N)
    //siendo N el numero de nodos
    int findk(int consulta){
        int aux=raiz->cv.clave;
        return buscak(consulta, raiz->tam_i, raiz);
    }
...
protected:
    ...
    int buscak(int consulta, int actual, Link a){
        if (consulta > nelems) return -1;
        else if (consulta == actual) return a->cv.clave;
        else if (consulta < actual) return buscak(consulta, actual-(a->tam_i-a->iz->tam_i) ,a->iz);
        else return buscak(consulta, actual+(a->dr->tam_i), a->dr);
    }
    ...
    static void rotaDer(Link & k2) {//k2 es a
        Link k1 = k2->iz;
        k2->iz = k1->dr;
        k1->dr = k2;
        ...
        k2 = k1;
        k2->dr->tam_i = k2->dr->tam_i - k2->tam_i;
    }
...
public:
    ...
    // iterador que sÌ permite modificar el elemento apuntado (su valor)
