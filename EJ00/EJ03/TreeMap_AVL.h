...
public:
    ...
    std::vector<int> range(int k1, int k2){
        std::vector<int> aux;
        rango(k1, k2, raiz, aux);
        return aux;
    }
protected:
    
    void rango(int k1, int k2, Link a, std::vector<int> & aux){
        if (a == nullptr)return;
        else{
            if (menor(a->cv.clave, k1))
                rango(k1, k2, a->dr, aux);
            else if (menor(k2, a->cv.clave)){
                rango(k1, k2, a->iz, aux);
            }
            else{
                rango(k1, k2, a->iz, aux);
                aux.push_back(a->cv.clave);
                rango(k1, k2, a->dr, aux);
            }
        }
    }
